"""Layer 1 (deck-specific) — Dragapult ex agent.

Vendored from kiyotah's official sample kernel
(data/samples/a-sample-rule-based-agent-dragapult-ex-deck), lightly adapted:
notebook magic + submission-packaging cell removed, `play` exposed for our
registry. Requires the official `cg.api` (Docker / mcts-style bundled submission).

This is a real deck-specific agent for the Dragapult ex deck — far stronger at
piloting it than the generic rule_based heuristic. Use as the competitive Layer-1
baseline while we design our own deck. Domain expert: tune the scoring here.
"""
...
# Registry entrypoint (make_safe handles deck-selection + crash safety).
play = agent
